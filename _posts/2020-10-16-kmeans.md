...
Such a group of data points is called a cluster.
...
These are called Voronoi cells and looks like this:
<figure>
...
Last but not least, finding the optimal number of clusters could quite tricky.
Business insights and prior knowledge can be very handy for this task.
To this day, more than 30 methods about finding the optimal number of cluster has been published.
...
This function is strictly decreasing. At first, when we add clusters,
it get easier to model the data. But after a certain point, increasing
the number of clusters brings little additional benefits
as it only split groups into sub-clusters. **The elbow method is a heuristic to
...
SSE can also be a stopping criteria for the algorithm.
If SSE is computed at the end of each iteration, a heuristic
could be stop the training when the SSE decreases less than a certain threshold
...
The *Elbow Method* is far from ideal. The SSE is a strickly decreasing function
and the optimal number of cluster is not mathematically defined.
Conclusively, a visual inspection in necessary to identify the elbow.
Consequently, the locating the elbow is subject (not all SSE curves will have as sharp elbow like in the example above).

**A good estimator of the model performance should take into account the quality of the model
but should also penalise for the complexity of the model.**

**Bayesian information criterion** (BIC) does exactly that. It is defined as:
\\( BIC = p\log(n) - 2 \log\left(\hat L (M; X) \right) \\)
where \\( M \\) is the model, \\( p\\) is the number of free-parameters of \\( M \\),
and \\( \hat L (M; X) \\) is the model likelihood.

For k-means, this translate to the following equation:

\\[ \begin{aligned} 
 BIC & = dk\log(n) + d(n-k) + nd\log(2\pi\sigma^2) - 2 \sum_{i=1}^{k} n_i \log\left(n_i\right) 
\\\\ \text{with}\; \sigma^2 & = \frac{SSE}{d(n-k)} 
\end{aligned} \\]

<div class="collapsible">Open Collapsible</div>
<div class="collapsible-content"><p>Lorem ipsum...</p></div> 


On k-means, BIC heavily relies on the assumption that cluster are identical spheres located at different locations.
The data is modeled with \\( k \\) gaussians at different locations \\( \textbf{c}_1, \, \ldots ,\; \textbf{c}_k \\)
respectively, but with the same variance \\( \sigma^2 \\). 

For a given partition of the space, the probability to observe a data point \\( \textbf{x} \\) is defined by:

\\[ P(\textbf{x} \vert S) =  \sum_{i=1}^{k} P(\textbf{x} \vert \textbf{x} \in S_i) P(\textbf{x} \in S_i) \\]

The last term, \\( P(\textbf{x} \in S_i) \\), the probability for the point to belong
to partition \\( S_i \\) is estimated by the proportion of training data points inside partition \\( S_i \\): 
\\( P(\textbf{x} \in S_i) = \frac{n_i}{n} \\)

The first term, \\( P(\textbf{x} \vert \textbf{x} \in S_i) \\) the probability to observe \\( \textbf{x} \\)
if it belongs to cluster \\( S_i \\), is a multivarita gaussian centered at \\(  \textbf{c}_i \\) with
variance \\( \sigma^2 \\) (recall the identical spherical hypothesis):

\\[ P(\textbf{x} \vert \textbf{x} \in S_i)  = 
\frac{1}{\sqrt{2 \pi \sigma^2}^{d}} \exp \left( -\frac{\vert \vert \textbf{x} - \textbf{c}_i \vert \vert_2^2}{2\sigma^2} \right) \\]

The log-likelihoods becomes:

\\[ \begin{aligned}
\log & \hat L(M; X)  = \log \prod_{\textbf{x} \in X} P(\textbf{x} \vert S) = \sum_{i=1}^{k} \sum_{\textbf{x} \in S_i} \log P(\textbf{x} \vert S)
...
The *maximum likelihood estimator* (MLE) gives an estimate of the variance
(calculus of the derivative is given as an exercice to the reader):


\\[ \hat \sigma^2 = \frac{1}{nd} \sum_{i=1}^{k} \sum_{\textbf{x} \in S_i} \vert\vert \textbf{x} - \textbf{c}_i \vert\vert^2 = \frac{SSE}{nd} \\]

In the meantine, the unbiaised estimator of the variance of each cluster give:

\\[ \forall 1\le i \le k, \; \hat\sigma_i^2 = \frac{1}{d(n_i - 1)} \sum_{\textbf{x} \in S_i} \vert\vert \textbf{x} - \textbf{c}_i \vert\vert^2 \\]

Combining the last two equations:

\\[ SSE = \sum_{i=1}^{k} \sum_{\textbf{x} \in S_i} \vert\vert \textbf{x} - \textbf{c}_i \vert\vert^2 = \sum\_{i=1}^k d(n_i - 1) \sigma_i^2 \\]

With the assumption of spherical clusters of same shape, we have \\( \hat\sigma^2 = \hat \sigma_i^2 \\). In conclusion,
the unbiaised estimation of the variance is: 

\\[ \hat\sigma^2 = \frac{1}{d(n-k)}  \sum_{i=1}^{k} \sum\_{\textbf{x} \in S_i} \vert\vert \textbf{x} - \textbf{c}_i \vert\vert^2 = \frac{SSE}{d(n-k)} \\]

Going back to the log-likelihood formula, it can be simplified:

\\[ \begin{aligned}
\log & \hat L(M; X) =  \sum_{i=1}^{k} \left[ n_i \log\left(\frac{n_i}{n}\right) \right] - \frac{d(n-k)}{2} - \frac{nd}{2}\log(2\pi\sigma^2)
\end{aligned} \\]


In the end, knowing that there are \\( p=k\times d \\) free parameters for k-means, the \\( BIC \\) expression becomes:
...
The overall time complexity at each step is therefore \\( O(nkd) \\).
...
<figure>
  <img src="/assets/img/outliers_off.png" alt="Voronoi no outliers" style="max-width:300px;display:inline-block;width:48%;">
  <img src="/assets/img/outliers_on.png" alt="Voronoi with outliers" style="max-width:300px;display:inline-block;width:48%;">
...
centroids, **the underlying assumption is that cluster are evenly sized only that direction**.
