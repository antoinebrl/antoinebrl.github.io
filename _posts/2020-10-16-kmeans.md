...
but should also penalise for the complexity of the model. Bayesian information criterion** (BIC) does exactly that. For k-means, it is defined as:
\\[ \begin{aligned} 
...
<details>
  <summary style="background:#eee">Click to expand if you like equations!</summary>
  <div style="background:#eee">

<p>
BIC is originally defined as: \( BIC = p\log(n) - 2 \log\left(\hat L (M; X) \right) \)
where \( M \) is the model, \( p \) is the number of free-parameters of \( M \),
and \( \hat L (M; X) \) is the model likelihood.
</p>

<p>
On k-means, BIC heavily relies on the assumption that clusters are identical spheres placed at different locations.
The \( k \) clusters are modeled with \( k \) gaussians, with means \( \textbf{c}_1, \, \ldots ,\; \textbf{c}_k \)
respectively, and the same variance \( \sigma^2 \).
</p>
...
if it belongs to cluster \( S_i \), is a multivariate gaussian:
...
With the underlying assumption of spherical clusters of same shape, we have \( \hat\sigma^2 = \hat \sigma_i^2 \). In conclusion,
