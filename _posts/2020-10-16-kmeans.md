...
don't have to. First stop K-means algorithm.
...
Non-supervised means that the data does not come with any label.
...
label ([supervised learning](https://en.wikipedia.org/wiki/Supervised_learning)).
...
of the optimal solution. The k-means algorithm, sometimes referred to as the Lloyd algorithm after his author {% cite lloyd1982least %},
...
These are called [Voronoi cells](https://en.wikipedia.org/wiki/Voronoi_diagram) and looks like this:
<figure>
...
Numpy {%cite harris2020array %} is a [Python library](https://numpy.org/) designed to ease the manipulation of 
...
I suggest you do not use this code if it's not for educational purpose. Scikit-learn {%cite scikit-learn %}
...
This function is strictly decreasing when applied on converged models.
At first, when we add clusters
...
**The *Elbow method* {% cite thorndike1953belongs %} is a heuristic to
...
Consequently, the estimation of the optimal number of cluster can be subjective.
...
but should also penalise for the complexity of the model. Bayesian Information Criterion** (*BIC*) {% cite schwarz1978estimating %}
and **Akaike Information Criterion** (*AIC*) {% cite akaike1974new %} measure the statistical quality of a model and respect this
parsimony principle (more parameters should not be added without necessity).
...
with a very high number of clusters {% cite friedman2001elements --label section --locator 7.7 %}:
...
\\(k'\\) leads to models with less clusters. It also shows that BIC also
...
<p>
On k-means, we can heavily relies on the assumption that data arises
...
I recommend this article {% cite hofmeyr2020degrees %} which
